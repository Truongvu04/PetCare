...
const VeterinaryListSidebar = ({
  ...
  loading = false,
  error = null,
  onRetry, // ✅ thêm props này để retry đúng
  searchRadius = 15000,
  onRadiusChange,
  locationError = null,
  onLocationRequest,
  ...
  const handleRadiusChange = (e) => {
    if (onRadiusChange) onRadiusChange(parseInt(e.target.value));
  };

  const renderErrorBox = (title, message, type = "error") => {
    const colorMap = {
      error: { bg: "bg-red-50", border: "border-red-200", icon: "error", text: "text-red-700" },
      warning: { bg: "bg-yellow-50", border: "border-yellow-200", icon: "warning", text: "text-yellow-700" },
    };
    const { bg, border, icon, text } = colorMap[type];
    return (
      <div className={`mx-4 mt-3 p-3 ${bg} border ${border} rounded-lg`}>
        <div className="flex items-start gap-2">
          <span className={`material-symbols-outlined ${text} text-lg mt-0.5`}>{icon}</span>
          <div className={`${text}`}>
  ...
  return (
    <div className="h-full flex flex-col bg-white">
      ...
      <div className="p-4 border-b border-gray-200 bg-gradient-to-r from-blue-600 to-green-600">
        <h1 className="text-xl font-bold text-white mb-1">Tìm Phòng Khám Thú Y</h1>
        <p className="text-blue-100 text-sm">Tìm kiếm phòng khám gần bạn với đánh giá cao</p>
      </div>
      ...
      {/* Error / Warning Boxes */}
      {locationError && renderErrorBox("Lỗi vị trí", locationError, "error")}
      {error &&
        renderErrorBox(
          "Lỗi tìm kiếm",
          <>
            ...
            {onRetry && (
              <button
                onClick={onRetry}
                className="ml-2 inline-block px-2 py-1 text-xs bg-red-600 text-white rounded hover:bg-red-700"
              >
                Thử lại
              </button>
            )}
        ...
        renderErrorBox(
          ...
          <>
            ...
            <button
              ...
              className="mt-1 px-2 py-1 bg-yellow-600 text-white text-xs rounded-lg hover:bg-yellow-700 transition-colors"
            >
              Cho phép
            </button>
          </>,
      ...
      {clinics.length > 0 && (
        <div className="p-4 border-t border-gray-200 bg-gray-50 text-center text-xs text-gray-600">
          Hiển thị {clinics.length} phòng khám trong bán kính{" "}
          {searchRadius / 1000}km
          {searchResults?.sources && (
            <div className="mt-1">
              Nguồn:{" "}
              {searchResults.sources.geoapify > 0 && "Geoapify"}
              {searchResults.sources.local > 0 &&
                searchResults.sources.geoapify > 0 &&
                " + "}
...
export default memo(VeterinaryListSidebar);
