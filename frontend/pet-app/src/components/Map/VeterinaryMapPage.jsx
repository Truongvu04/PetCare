...
const VeterinaryMapPage = () => {
  ...
  const handleEnhancedSearch = async (searchOptions) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Update map center if location from search
        if (result.location) {
  ...
  // Handle clinic selection from sidebar
  ...
  const handleLocationSelect = (coordinates, address) => {
    ...
    // Fetch clinics near selected location
  ...
  // Determine current user location for map
