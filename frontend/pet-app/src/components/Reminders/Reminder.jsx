...
const RemindersAuto = () => {
  ...
  useEffect(() => {
    ...
    const currentMinTime = `${hours}:${minutes}`;
    setMinTime(currentMinTime);
    
    if (fFreq === 'none' && feedingTime && feedingTime < currentMinTime) {
        setFeedingTime('');
    }
  }, [user, fFreq, feedingTime]);
  ...
  async function handleSubmit(e) {
    ...
    // Feeding
    if (fPet && feedingTime && fFreq === 'none') { 
        if (feedingTime < minTime) {
            alert("Feeding time must be in the future for today. Please select a later time.");
            return;
        }
    }
            ...
            <form className="space-y-12" onSubmit={handleSubmit}>
              ...
              <section className="bg-white p-8 rounded-2xl shadow-md border border-gray-100 hover:shadow-lg transition">
                ...
                <div className="space-y-5">
                  ...
                  <input type="time" value={feedingTime} onChange={(e) => setFeedingTime(e.target.value)} min={fFreq === 'none' ? minTime : undefined} className="w-full bg-green-50 rounded-xl p-3 text-sm text-gray-800 focus:ring-2 focus:ring-green-400 border border-gray-200 focus:outline-none focus:border-green-300" />
