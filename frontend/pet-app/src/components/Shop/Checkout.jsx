...
const Checkout = () => {
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      // Build order payload (include payment method for frontend flow)
      const orderPayload = {
        cartItems: cartItemsForAPI,
      ...
      const orderId = orderResp.data.order_id || orderResp.data.orderId;
      // 2) if payment method is vnpay -> request payment url and redirect
      if (orderPayload.payment_method === "vnpay") {
        const payResp = await api.post("/payments/vnpay/create", { orderId });
      ...
      clearCart();
      navigate(`/order-confirmation`, { state: { orderId: orderResp.data.order_id } });
    } catch (error) {
