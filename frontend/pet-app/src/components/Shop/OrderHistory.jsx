...
import { Package, Calendar, ArrowRight, Loader2, CheckCircle, X } from "lucide-react";
...
const OrderHistory = () => {
  ...
  const handleCancelOrder = async (orderId, e) => {
    ...
    try {
      ...
      showSuccess("Success", "Order cancelled successfully! Your refund will be processed shortly.");
      // Refresh orders list
      await fetchOrders();
    } catch (error) {
      console.error("Error cancelling order:", error);
      const message = error.response?.data?.message || "Failed to cancel order. Please try again.";
      showError("Error", message);
    } finally {
  ...
  const isCancellable = (order) => {
    // Only allow cancellation for pending, paid, confirmed, processing statuses
    const cancellableStatuses = ["pending", "paid", "confirmed", "processing"];
    if (!cancellableStatuses.includes(order.status)) return false;

    // Only allow cancellation if order is less than 24 hours old
              ...
              <div
                ...
                <div className="flex justify-between items-start">
                  ...
                  <div className="text-right ml-6 flex flex-col items-end gap-3">
                    ...
                    {/* Cancel Order Button - Show for cancellable orders */}
                    {isCancellable(order) && (
                      <button
                        onClick={(e) => handleCancelOrder(order.order_id, e)}
                        disabled={cancelingOrder === order.order_id}
                        className="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors font-medium flex items-center gap-2 shadow-md shadow-red-200 disabled:opacity-70 disabled:cursor-not-allowed"
                      >
